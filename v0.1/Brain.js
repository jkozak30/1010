class Brain {
    ...
    bestMove() {
        ...
        for (var i=0; i<=10-pieces[0].h/30; i++) {
            for (var j=0; j<=10-pieces[0].w/30; j++) {
                
                for (var k=0; k<=10-pieces[1].h/30; k++) {
                    for (var l=0; l<=10-pieces[1].w/30; l++) {

                        for (var m=0; m<=10-pieces[2].h/30; m++) {
                            for (var n=0; n<=10-pieces[2].w/30; n++) {

                                if (this.hypCheckMove(0, i, j)) {
                                    this.hypMove(0, i, j); this.hypCheck();
                                    if (this.hypCheckMove(1, k, l)) {
                                        this.hypMove(1, k, l); this.hypCheck();
                                        if (this.hypCheckMove(2, m, n)) {
                                            this.hypMove(2, m, n); this.hypCheck();
                                            if (rs[0] == -1) {
        ...
        console.log(rs[0]);
        if (rs[0] == -1) {return;}
        this.move(0, rs[0], cs[0]); board.check(); this.move(1, rs[1], cs[1]); board.check(); this.move(2, rs[2], cs[2]); board.check(); 
